...
import { BreedAdaptabilityLevel, TypesAdaptabilityLevel } from "@/breeds/domain/breed.adaptability.level";
...
export class Breed extends Entity {
  ...
  constructor(name: BreedName, adaptabilityLevel: BreedAdaptabilityLevel, coatLength: BreedCoatLength) {
    super();
    this.name = name;
  ...
  static fromPrimitives(name: string, adaptabilityLevel: string, coatLength: string) {
    const breedName = BreedName.fromValue(name);
    const breedAdaptabilityLevel = BreedAdaptabilityLevel.fromValue(adaptabilityLevel);
